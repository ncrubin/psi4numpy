    ...
    def generateMatrix(self, detList):
        ...
        for i in range(numDet):
            if i % 500 == 0:
                ...
                start_time = time.time()
            for j in range(i + 1):
                ijval = self.calcMatrixElement(detList[i], detList[j])
                if not np.isclose(abs(ijval), 0.0):
                    if i != j:
                        nz_row_idx.append(i)
                        nz_row_idx.append(j)
                        nz_col_idx.append(j)
                        nz_col_idx.append(i)
                        val_idx.append(ijval)
                        val_idx.append(ijval)
                    else:
                        nz_row_idx.append(i)
                        nz_col_idx.append(j)
        ...
        matrix = csc_matrix((val_idx, (nz_row_idx, nz_col_idx)), shape=(numDet, numDet))
        return matrix
